    ...
    <div class="container flex flex-row flex-wrap items-center justify-between mx-auto md:justify-center">
        ...
        <nav class="absolute left-0 z-20 flex-col justify-end flex-auto w-full font-semibold bg-gray-100 rounded-lg shadow-md select-none font-primary lg:justify-start md:ml-4 dark:bg-gray-800 md:relative top-16 md:top-0 md:flex md:flex-row md:space-x-2 md:w-auto md:bg-transparent md:shadow-none" :class="{ 'flex' : mobileMenuOpen , 'hidden' : !mobileMenuOpen}">
            <div @click="topMenuOpen = !topMenuOpen" @click.away="topMenuOpen = false" class="relative text-gray-600 rounded-lg cursor-pointer dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-900">
                <div class="flex flex-row items-center justify-between gap-4 p-3 rounded-lg dark:bg-gray-800 text-link dark:hover:bg-gray-900 dark:text-white" :class="{'bg-gray-300 text-blue-700 dark:text-blue-300 dark:bg-gray-900': topMenuOpen}">
                    <p>Top Anime</p>
                ...
                <div x-show="topMenuOpen" class="relative left-0 w-full bg-gray-300 border-gray-400 rounded-lg shadow-xl dark:border-gray-600 md:border md:origin-top-left md:absolute md:w-56 dark:bg-gray-800 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                    <div role="none">
                        <a href="{{ route('anime.top.rated') }}" class="block px-4 py-2 text-sm rounded-lg text-link dark:text-white hover:bg-gray-400 dark:hover:bg-gray-900" role="menuitem" tabindex="-1" >
                            {{ __('anime.top.title.rated') }}
                        </a>
                        <a href="{{ route('anime.top.popular') }}" class="block px-4 py-2 text-sm rounded-lg text-link dark:text-white hover:bg-gray-400 dark:hover:bg-gray-900" role="menuitem" tabindex="-1" >
                            {{ __('anime.top.title.popularity') }}
                        </a>
                        <a href="{{ route('anime.top.airing') }}" class="block px-4 py-2 text-sm rounded-lg text-link dark:text-white hover:bg-gray-400 dark:hover:bg-gray-900" role="menuitem" tabindex="-1" >
                            {{ __('anime.top.title.airing') }}
                        </a>
                        <a href="{{ route('anime.top.upcoming') }}" class="block px-4 py-2 text-sm rounded-lg text-link dark:text-white hover:bg-gray-400 dark:hover:bg-gray-900" role="menuitem" tabindex="-1" >
                            {{ __('anime.top.title.upcoming') }}
                        </a>
                    </div>
                </div>
            </div>
            <a href="{{ route('anime.season-current') }}" class="p-3 text-gray-600 rounded-lg dark:text-white text-link hover:bg-gray-300 dark:hover:bg-gray-900">
                {{ __('anime.season.title') }}
            </a>
            <a href="{{ route('anime.genre') }}" class="p-3 text-gray-600 rounded-lg dark:text-white text-link hover:bg-gray-300 dark:hover:bg-gray-900">
